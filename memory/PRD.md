...
- [x] **Flyer Auto-Post to Social Wall** (Feb 2026) — when an event flips from Hidden → Active, backend auto-creates a Social Wall photo post with the flyer image and a formatted caption (`🎉 Name\nDate · Time · Location\nDescription`). Location targeting: single slug → single-location post; `all-locations` or blank → fans out to every active non-hibachi location under one `broadcast_id`. `wall_post_ids` are stored on the event so re-toggling never duplicates. Auto-post also mirrors to `gallery_items` matching DJ post behavior. Verified with curl (14 → 15 posts on activate, stays 15 on re-toggle).
- [x] **Bulk Events Sync to Prod** (Feb 2026) — added `/app/backend/migrations/events_sync.json` (git-tracked). Every create / bulk-create / update writes the event to this snapshot; delete purges it. New `apply_events_sync_migration()` in `database.py` runs on FastAPI startup and inserts only events whose `id` is not already in the DB — so prod admin edits are preserved and new preview bulk-uploads land on the next deploy. `wall_post_ids` and `wall_posted_at` are stripped during sync since posts live per-DB. Verified: seeded snapshot with a synthetic event → restart → event appears in DB with `wall_post_ids` correctly nulled.
