...
@api_router.get("/wall/posts/{location_slug}")
async def get_wall_posts(location_slug: str, limit: int = 50, skip: int = 0):
    """Get social wall posts for a location"""
...
@api_router.post("/wall/posts/{post_id}/comment")
...
async def delete_wall_post(post_id: str, user_id: str = ""):
    """Delete a wall post (only by author)"""
    post = await db.wall_posts.find_one({"id": post_id}, {"_id": 0})
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
...
@api_router.post("/wall/posts/upload-image")
...
@api_router.post("/wall/chat/{location_slug}")
async def send_chat_message(location_slug: str, body: dict):
    """Send a message to a location's group chat"""
    user_id = body.get("user_id")
    content = body.get("content", "").strip()
    if not user_id or not content:
        raise HTTPException(status_code=400, detail="user_id and content required")
...
@api_router.get("/wall/chat/{location_slug}")
async def get_chat_messages(location_slug: str, limit: int = 100, before: str = ""):
    """Get group chat messages for a location"""
    query = {"location_slug": location_slug}
    if before:
        query["created_at"] = {"$lt": before}
    cursor = db.wall_chat_messages.find(
        query, {"_id": 0}
...
@api_router.get("/wall/dm/conversations/{user_id}")
async def get_dm_conversations(user_id: str):
        ...
        {"$group": {
            ...
            "partner_avatar": {
                "$first": {
                    "$cond": [
                        {"$eq": ["$from_user_id", user_id]},
                        {"$ifNull": ["$to_user_avatar", ""]},
                        {"$ifNull": ["$from_user_avatar", ""]}
                    ]
                }
            },
            "unread": {
    ...
    conversations = await db.wall_dm_messages.aggregate(pipeline).to_list(length=50)
    return conversations


@api_router.get("/wall/dm/thread/{user_id}/{partner_id}")
async def get_dm_thread(user_id: str, partner_id: str, limit: int = 50):
    """Get DM thread between two users"""
...
@api_router.get("/wall/dm/unread/{user_id}")
async def get_dm_unread_count(user_id: str):
    """Get unread DM count"""
    count = await db.wall_dm_messages.count_documents(
        {"to_user_id": user_id, "read": False}
    )
    return {"unread": count}


@api_router.get("/wall/users/{location_slug}")
async def get_wall_users(location_slug: str):
    """Get recent active users at a location (for DM target selection)"""
    # Get users who posted or chatted at this location recently
